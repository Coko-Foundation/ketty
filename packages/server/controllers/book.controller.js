  ...
  try {
    ...
    return useTransaction(
      async tr => {
        ...
        const { result: associatedTeams } = await getTeams(
          { objectId: bookId },
          { trx: tr },
        )
    ...
    if (includePdf) {
      ...
      const pdfFile = await exporter(
        ...
        null, // icmlNotes
        {
          ...
          includeTOC: pdfProfile.includedComponents.toc,
          includeCopyrights: pdfProfile.includedComponents.copyright,
    ...
    if (includeEpub) {
      ...
      const epubFile = await exporter(
        ...
        null, // icmlNotes
        {
          ...
          includeTOC: epubProfile.includedComponents.toc,
          includeCopyrights: epubProfile.includedComponents.copyright,
...
const getBookTOC = async bookId => {
  ...
  chapters.forEach(item => {
    if (item.parentComponentId) {
      const parent = itemMap[item.parentComponentId]

      if (parent) {
        parent.children.push(
          omitBy(
            itemMap[item.id],
            (_, k) => k === 'parentComponentId' || k === 'children',
          ),
        )
      }
    } else {
      nestedResults.push(
        omitBy(itemMap[item.id], (_, k) => k === 'parentComponentId'),
      )
    }
  })

  // return nested results and omit children field for chapters
  return nestedResults.map(r =>
    r.type === 'chapter' ? omitBy(r, (v, k) => k === 'children') : r,
  )
}
