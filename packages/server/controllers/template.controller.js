...
const cloneTemplate = async (id, name, cssFile, hashed, options = {}) => {
  try {
    ...
    return useTransaction(
      async tr => {
        const tempFolder = `${tempFolderPath}/previewer`
        ...
        await fs.remove(`${tempFolderPath}/previewer/${hashed}`)
        return newTemplate
      },
      { trx },
    )
  ...
  try {
    ...
    return useTransaction(
      async tr => {
        ...
        fs.writeFileSync(
          path.join(tempFolderPath, 'previewer', hashed, name),
          data,
        )
        ...
        await fs.remove(path.join(tempFolderPath, 'previewer', hashed))
        ...
        const tempaltesCreated = await persistTemplates(
          {
            ...
            author,
            thumbnailFile,
            supportedNoteTypes: ['chapterEnd'], // hardcoded for now, will find out what it means later
            ignoreConfig: true,
            sourceRoot,
          },
          {
