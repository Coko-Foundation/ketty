...
const getEntityFilesHandler = async (_, { input }) => {
  try {
...
const getSpecificFilesHandler = async (_, { ids }) => {
  try {
...
const getFileHandler = async (_, { id }) => {
  try {
    return getFile(id)
  } catch (e) {
    logger.error(e)
...
const uploadFilesHandler = async (_, { files, entityId }) => {
  try {
...
const deleteFilesHandler = async (_, { ids, remoteToo }) => {
  try {
    let deletedFiles
...
module.exports = {
  ...
  File: {
    ...
    // async inUse({ id, mimetype, bookId }) {
    //   let inUse = []
