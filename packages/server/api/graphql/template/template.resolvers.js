...
const getTemplateHandler = async (_, { id }) => {
...
const createTemplateHandler = async (_, { input }) => {
  try {
    const {
      name,
      author,
...
const updateTemplateHandler = async (_, { input }) => {
  try {
...
const deleteTemplateHandler = async (_, { id }) => {
  try {
...
const getExportScriptsHandler = async (_, { scope }) => {
  try {
...
module.exports = {
  ...
  Template: {
    async files(template, _) {
      const files = await template.getFiles()
      return files
    },
    async thumbnail(template) {
      try {
