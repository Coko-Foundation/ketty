...
const BookComponentStateLoader = require('../../../models/dataloader/loaders/bookComponentStateLoader')
...
const getBookComponentHandler = async (_, { id }) => {
...
const ingestWordFileHandler = async (_, { bookComponentFiles }) => {
  try {
...
const addBookComponentHandler = async (_, { input }) => {
  try {
...
const podAddBookComponentHandler = async (_, { input }) => {
  try {
...
const renameBookComponentHandler = async (_, { input }) => {
  try {
...
// used in ketty
const renameBookComponentTitleHandler = async (_, { id, title }) => {
  try {
    await renameBookComponent(id, title, 'en')
...
const deleteBookComponentHandler = async (_, { input }) => {
  try {
    const { id } = input
    const bookComponent = await getBookComponent(id)
...
const podDeleteBookComponentHandler = async (_, { input }) => {
  try {
    const { id } = input
    const bookComponent = await getBookComponent(id)
...
const updateContentHandler = async (_, { input }) => {
  try {
...
const updatePaginationHandler = async (_, { input }) => {
  try {
...
const updateTrackChangesHandler = async (_, { input }) => {
  try {
...
const updateUploadingHandler = async (_, { input }) => {
  try {
...
const updateComponentTypeHandler = async (_, { input }) => {
  try {
...
const updateBookComponentParentIdHandler = async (_, { input }) => {
  try {
...
const toggleIncludeInTOCHandler = async (_, { input }) => {
  try {
    const { id } = input
...
const setBookComponentStatusHandler = async (_, { id, status }) => {
  try {
...
module.exports = {
  ...
  BookComponent: {
    ...
    async bookId(bookComponent) {
      return bookComponent.bookId
    },
    async status(bookComponent) {
      const bookComponentState = await BookComponentState.findOne({
    ...
    async trackChangesEnabled(bookComponent) {
      const bookComponentState = await BookComponentState.findOne({
        bookComponentId: bookComponent.id,
      })
    ...
    async hasContent(bookComponent) {
      const bookComponentTranslation = await BookComponentTranslation.findOne({
        bookComponentId: bookComponent.id,
        languageIso: 'en',
      })
    ...
    async componentTypeOrder(bookComponent, _, ctx) {
      ...
      const sortedPerDivision = await DivisionLoader.bookComponents.load(
    ...
    async uploading(bookComponent, _, ctx) {
      await BookComponentStateLoader.state.clear()

      const bookComponentState = await BookComponentStateLoader.state.load(
        bookComponent.id,
      )

      return bookComponentState.uploading
    },
    async pagination(bookComponent) {
      return bookComponent.pagination
    },
    async workflowStages(bookComponent, _, ctx) {
      BookComponentStateLoader.state.clear()
    ...
    async languages(bookComponent) {
      const bookComponentTranslations = await BookComponentTranslation.find({
        bookComponentId: bookComponent.id,
      })

      return bookComponentTranslations.result.map(
