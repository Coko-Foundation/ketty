...
- Optionally edit or add to the text you've selected in the 'Content' area.
- Type your query in the promt area and press enter or select the 'Send' icon.
- The response may include the following:
  - The response text in the 'Content' area.
  - Citations that are present in the searched files, if relevant, and the filename that was queired in the KB.
  - Links that are present in the searched files, if relevant.

:::note

When the KB functionality is enabled in Book Settings, the AI-assistant will query the KB by default. You can choose to _disable_ a KB search when relevant by disable the 'Ask Knowelege Base' button.
:::
